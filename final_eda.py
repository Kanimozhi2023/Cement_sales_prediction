...
lagged_Values = train1.values[-8:]
pred = result.forecast(y=lagged_Values, steps=12) 
pred
test1.head()
idx = pd.date_range(start='1/8/2019',periods=12,freq='MS')
df_forecast = pd.DataFrame(pred,index=idx,columns=['sales','demand','priceofproduct','limestonerequirement','noofhousesbuilt','gdpofindiaincrores'])
df_forecast[:5]

sns.lineplot(data=df_forecast['sales'],label="var_pred")
sns.lineplot(data=test1['sales'],label='original_sales')
